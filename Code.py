...
def extract_players(player_data):
    ...
    players_grouped['Clutch_Ratio'] = players_grouped.apply(lambda row: row['won'] / row['played'] if row['played'] != 0 else 0, axis=1)
...
for year in year_list:
    ...
    match_results['Win'] = match_results[['Elimination', 'Detonated', 'Defused', 'Time Expiry (No Plant)']].sum(axis=1) == 13
...
# could add a restriction above to limit it to only certain match types (eg finals, semis, etc)
# we would then need to convert player names to their labels from above - convert to a list or something so we can have multiples of 1 label
# do stats analysis - maybe data mining? not sure
